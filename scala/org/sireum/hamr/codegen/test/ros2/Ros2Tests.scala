      ...
      if (ros2SetupPath.nonEmpty) {
        ...
        if (interfacesResults.ok) {
          val bringUpResults = Os.proc(ISZ("bash", "-c",
              s"source ${ros2SetupPath.get.value}; env; python3 --version && which python3; colcon build --cmake-args -DCMAKE_CXX_FLAGS=\"-w\" --packages-select $pkgName ${pkgName}_bringup"))
            .at(resultsRoot / testName / "results" / strictModeString).echo.run()
