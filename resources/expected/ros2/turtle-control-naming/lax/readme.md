...
# turtle_control_naming — Mixed ROS2 / microROS Workspace
...
|---|---|---|---|
| `proc_pilot_exe` | `turtle_control_naming_cpp_pkg` | ROS2 (rclcpp) | Sporadic |
| `proc_encoder_exe` | `turtle_control_naming_microros_pkg` | microROS (rclc + rmw_microxrcedds) | Periodic |
...
cp -r src/turtle_control_naming_cpp_pkg_interfaces $MICROROS_WS/src/
cp -r microros_apps/turtle_control_naming_microros_pkg $MICROROS_WS/src/
cd $MICROROS_WS && colcon build --packages-select turtle_control_naming_cpp_pkg_interfaces turtle_control_naming_microros_pkg
...
RMW_IMPLEMENTATION=rmw_microxrcedds ros2 run turtle_control_naming_microros_pkg proc_encoder_exe

# proc_pilot — ROS2 node
ros2 run turtle_control_naming_cpp_pkg proc_pilot_exe
```
