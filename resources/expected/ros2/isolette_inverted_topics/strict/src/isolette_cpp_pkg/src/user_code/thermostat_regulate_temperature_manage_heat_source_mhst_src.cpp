...
void thermostat_regulate_temperature_manage_heat_source_mhst::timeTriggered()
{
    ...
    isolette_cpp_pkg_interfaces::msg::Tempimpl lower_desired_temp = get_lower_desired_temp();
    PRINT_INFO("Received lower_desired_temp: %s", MESSAGE_TO_STRING(lower_desired_temp));
    isolette_cpp_pkg_interfaces::msg::Tempimpl upper_desired_temp = get_upper_desired_temp();
