...
# Brings up the whole system on a host.  Launch UrosDemoSystem_Instance_ros2.launch.py on its own when
...
def generate_launch_description():
    ...
    ld.add_action(IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory("uros_demo_structure_cpp_pkg_bringup"),
                "launch",
                "UrosDemoSystem_Instance_ros2.launch.py"))))
    ...
    ld.add_action(IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory("uros_demo_structure_cpp_pkg_bringup"),
                "launch",
                "UrosDemoSystem_Instance_microros.launch.py"))))
