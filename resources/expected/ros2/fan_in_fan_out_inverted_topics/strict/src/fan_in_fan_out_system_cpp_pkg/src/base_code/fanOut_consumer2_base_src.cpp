...
fanOut_consumer2_base::fanOut_consumer2_base() : Node("fanOut_consumer2")
{
    ...
    fanOut_consumer2_myInteger_subscription_ = this->create_subscription<fan_in_fan_out_system_cpp_pkg_interfaces::msg::Integer64>(
        ...
        1,
        std::bind(&fanOut_consumer2_base::accept_myInteger, this, std::placeholders::_1), subscription_options_);
...
void fanOut_consumer2_base::accept_myInteger(fan_in_fan_out_system_cpp_pkg_interfaces::msg::Integer64 msg)
{
...
void fanOut_consumer2_base::handle_myInteger_base(MsgType msg)
{
...
void fanOut_consumer2_base::receiveInputs(std::queue<MsgType>& infrastructureQueue, std::queue<MsgType>& applicationQueue) {
...
void fanOut_consumer2_base::enqueue(std::queue<MsgType>& queue, MsgType val) {
    if (queue.size() >= 1) {
        queue.pop();
    }
    queue.push(val);
...
void fanOut_consumer2_base::sendOutputs() {
    for (std::tuple<std::queue<MsgType>*, std::queue<MsgType>*, void (fanOut_consumer2_base::*)(MsgType)> port : outPortTupleVector) {
