...
isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat_base::isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat_base() : Node("isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat")
{
    ...
    periodTimer_ = this->create_wall_timer(std::chrono::milliseconds(1000),
        std::bind(&isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat_base::timeTriggeredCaller, this), cb_group_);
    ...
    // Used by sendOutputs
    outPortTupleVector = {
        {&applicationOut_current_tempWstatus, &infrastructureOut_current_tempWstatus, &isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat_base::sendOut_current_tempWstatus}
    };
...
void isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat_base::timeTriggeredCaller() {
    receiveInputs();
    timeTriggered();
    sendOutputs();
}

void isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat_base::receiveInputs() {
...
void isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat_base::enqueue(std::queue<MsgType>& queue, MsgType val) {
    if (queue.size() >= 1) {
        queue.pop();
    }
    queue.push(val);
...
void isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat_base::sendOutputs() {
    for (std::tuple<std::queue<MsgType>*, std::queue<MsgType>*, void (isolette_single_sensor_Instance_temperature_sensor_cpi_thermostat_base::*)(MsgType)> port : outPortTupleVector) {
