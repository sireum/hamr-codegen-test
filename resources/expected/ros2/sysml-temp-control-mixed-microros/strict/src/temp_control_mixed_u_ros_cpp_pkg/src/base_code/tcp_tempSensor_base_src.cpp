...
tcp_tempSensor_base::tcp_tempSensor_base() : Node("tcp_tempSensor")
{
    ...
    tcp_tempSensor_currentTemp_publisher_1 = this->create_publisher<temp_control_mixed_u_ros_cpp_pkg_interfaces::msg::Temperature>(
        "tcp_tempControl_currentTemp",
        1);

    tcp_tempSensor_currentTemp_publisher_2 = this->create_publisher<temp_control_mixed_u_ros_cpp_pkg_interfaces::msg::Temperature>(
        "tcp_opInterface_currentTemp",
        1);

    tcp_tempSensor_tempChanged_publisher_ = this->create_publisher<temp_control_mixed_u_ros_cpp_pkg_interfaces::msg::Empty>(
...
{
    if (auto typedMsg = std::get_if<temp_control_mixed_u_ros_cpp_pkg_interfaces::msg::Temperature>(&msg)) {
        tcp_tempSensor_currentTemp_publisher_1->publish(*typedMsg);
        tcp_tempSensor_currentTemp_publisher_2->publish(*typedMsg);
    } else {
...
{
    if (auto typedMsg = std::get_if<temp_control_mixed_u_ros_cpp_pkg_interfaces::msg::Empty>(&msg)) {
...
void tcp_tempSensor_base::put_currentTemp(temp_control_mixed_u_ros_cpp_pkg_interfaces::msg::Temperature msg)
{
...
{
    enqueue(applicationOut_tempChanged, temp_control_mixed_u_ros_cpp_pkg_interfaces::msg::Empty());
}
