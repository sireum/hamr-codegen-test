...
# Brings up the whole system on a host.  Launch TempControlSystem_Instance_ros2.launch.py on its own when
...
def generate_launch_description():
    ...
    ld.add_action(IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory("temp_control_mixed_u_ros_cpp_pkg_bringup"),
                "launch",
                "TempControlSystem_Instance_ros2.launch.py"))))
    ...
    ld.add_action(IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory("temp_control_mixed_u_ros_cpp_pkg_bringup"),
                "launch",
                "TempControlSystem_Instance_microros.launch.py"))))
