...
static void tcp_tempControl_sendOutputs(tcp_tempControl_base_t * self)
{
    if (self->tcp_tempControl_fanCmd_out_hasValue) {
        rcl_ret_t ret = rcl_publish(&self->tcp_tempControl_fanCmd_publisher, &self->tcp_tempControl_fanCmd_out, NULL);
        if (ret != RCL_RET_OK) {
            LOG_ERROR("Failed to publish fanCmd");
        }
        self->tcp_tempControl_fanCmd_out_hasValue = false;
    }
}
...
static void tcp_tempControl_currentTemp_subscription_callback(const void * msgin)
{
    ...
    // executor has already written that value into tcp_tempControl_currentTemp_msg: it is the
    // buffer handed to rclc_executor_add_subscription, so rcl_take fills it before
    // this callback is invoked, and get_currentTemp returns that same storage.
    //
...
static void tcp_tempControl_tempChanged_subscription_callback(const void * msgin)
{
    ...
    if (g_self != NULL) {
        ...
        tcp_tempControl_sendOutputs(g_self);
    }
}
...
{
    self->tcp_tempControl_fanCmd_out = *msg;
    self->tcp_tempControl_fanCmd_out_hasValue = true;
}
