...
BuildingControlDemo_i_Instance_tcp_tempControl_base::BuildingControlDemo_i_Instance_tcp_tempControl_base() : Node("BuildingControlDemo_i_Instance_tcp_tempControl")
{
    ...
    // Used by sendOutputs
    outPortTupleVector = {
        {&applicationOut_fanCmd, &infrastructureOut_fanCmd, &BuildingControlDemo_i_Instance_tcp_tempControl_base::sendOut_fanCmd}
    };
...
void BuildingControlDemo_i_Instance_tcp_tempControl_base::receiveInputs(std::queue<MsgType>& infrastructureQueue, std::queue<MsgType>& applicationQueue) {
...
void BuildingControlDemo_i_Instance_tcp_tempControl_base::enqueue(std::queue<MsgType>& queue, MsgType val) {
    if (queue.size() >= 1) {
        queue.pop();
    }
    queue.push(val);
...
void BuildingControlDemo_i_Instance_tcp_tempControl_base::sendOutputs() {
    for (std::tuple<std::queue<MsgType>*, std::queue<MsgType>*, void (BuildingControlDemo_i_Instance_tcp_tempControl_base::*)(MsgType)> port : outPortTupleVector) {
