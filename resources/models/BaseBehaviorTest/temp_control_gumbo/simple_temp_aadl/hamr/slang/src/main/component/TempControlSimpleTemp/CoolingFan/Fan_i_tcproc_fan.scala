...
object Fan_i_tcproc_fan {
  ...
  def initialise(api: Fan_i_Initialization_Api): Unit = {
    Contract(
      // This requires clause should be implicit.
      // It is an axiom that output event data ports start off empty
      Requires(api.fanAck == None[FanAck.Type]()),
      Ensures(api.fanAck == None[FanAck.Type]())
    )
  ...
  def handle_fanCmd(api: Fan_i_Operational_Api, value : CoolingFan.FanCmd.Type): Unit = {
    Contract(
      ...
      // It is an axiom that output event data ports start off empty
      Requires(api.fanAck == None[FanAck.Type]()),
      Ensures(api.fanAck.nonEmpty) // i.e., a message must be sent on the fanAck port
    )
