...
Unit a_SharedMemory_receive(STACK_FRAME Z port, MBox2_1CBFC4 out) {
...
Unit a_SharedMemory_receiveAsync(STACK_FRAME Z port, MBox2_1029D1 out) {
    int sid = semget((key_t) port, 1, 0666);
...
Unit a_SharedMemory_send(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
...
B a_SharedMemory_sendAsync(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
