...
Z a_SharedMemory_create(STACK_FRAME Z id) {
    unsigned int permission = 0666;
    unsigned int mask = IPC_CREAT;
...
Unit a_SharedMemory_receive(STACK_FRAME Z port, MBox2_43CC67 out) {
...
Unit a_SharedMemory_receiveAsync(STACK_FRAME Z port, MBox2_37E193 out) {
    int sid = semget((key_t) port, 1, 0666);
...
Unit a_SharedMemory_send(STACK_FRAME Z appPortId, Z componentPortId, art_DataContent d) {
...
B a_SharedMemory_sendAsync(STACK_FRAME Z appPortId, Z componentPortId, art_DataContent d) {
    int sid = semget((key_t) componentPortId, 1, 0666);
