...
object TempSensor_s_tcproc_tempSensor_SystemTestAPI {
  ...
  def fetchContainer(): tc.TempSensor.TempSensor_s_tcproc_tempSensor_PostState_Container_PS = {
    if (runtimeMonitorStream.contains(Arch.TempControlSoftwareSystem_s_Instance_tcproc_tempSensor.id)) {
      val (_, postContainer_) = runtimeMonitorStream.get(Arch.TempControlSoftwareSystem_s_Instance_tcproc_tempSensor.id).get
      return postContainer_.asInstanceOf[tc.TempSensor.TempSensor_s_tcproc_tempSensor_PostState_Container_PS]
    }
    else {
      assert(F, s"No post state recorded for ${Arch.TempControlSoftwareSystem_s_Instance_tcproc_tempSensor.name}")
      halt(s"No post state recorded for ${Arch.TempControlSoftwareSystem_s_Instance_tcproc_tempSensor.name}")
    }
  }
