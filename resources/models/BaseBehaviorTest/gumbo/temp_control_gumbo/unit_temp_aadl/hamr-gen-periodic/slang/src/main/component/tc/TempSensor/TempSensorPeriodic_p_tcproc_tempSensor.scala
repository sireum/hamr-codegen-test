...
@ext("TempSensorDevice_Ext_Sim") object TempSensorDevice {
  def currentTempGet(): Temperature_i = Contract.Only(
    Ensures(
      GUMBO_Definitions.GUMBO__Library.isFahrenheit(Res[Temperature_i]),
      Res[Temperature_i].degrees >= -50.0f & Res[Temperature_i].degrees <= 150.0f,

      // FIXME: see similar error/issue in the sporadic version slang source file
      TempSensorPeriodic_p_Api.Sensor_Temperature_Range(Res[Temperature_i])
    )
  )
}
