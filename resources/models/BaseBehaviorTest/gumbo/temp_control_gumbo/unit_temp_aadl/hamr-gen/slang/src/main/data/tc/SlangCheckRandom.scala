...
@msig trait RandomLibI {
  ...
  def next_TempSensorUnitType(): TempSensor.Unit.Type = {

    var ordinal: Z = gen.nextZBetween(0, tc.TempSensor.Unit.numOfElements-1) //is genBetween inclusive
  ...
  def next_ISZ_TempSensorUnit_Payload(): ISZ[TempSensor.Unit_Payload] = {
     assert(F, "Requirements to strict to generate")
     halt("Requirements to strict to generate")
    }

  def next_TempSensorUnit_Payload(): TempSensor.Unit_Payload = {
    var value: TempSensor.Unit.Type = next_TempSensorUnitType()
  ...
  def next_TempSensorTemperature_i(): TempSensor.Temperature_i = {
    ...
    var unit: TempSensor.Unit.Type = next_TempSensorUnitType()
...
@record class RandomLib(val gen: org.sireum.Random.Gen) extends RandomLibI {
  ...
  var config_TempSensorUnitType: Config_TempSensorUnitType = Config_TempSensorUnitType(alwaysTrue_TempSensorUnitType _)
  ...
  var config_TempSensorUnit_Payload: Config_TempSensorUnit_Payload = Config_TempSensorUnit_Payload(alwaysTrue_TempSensorUnit_Payload _)
