...
object Producer_p_producer_GumboX_TestHarness_ScalaTest_Generator {
  ...
  def serializeReport(dir: Os.Path): Unit = {
    val filename = dir / s"Producer_p_producer_GumboX_TestHarness_ScalaTest_Generator_${System.currentTimeMillis()}.json"
...
import Producer_p_producer_GumboX_TestHarness_ScalaTest_Generator._

trait Producer_p_producer_GumboX_TestHarness_ScalaTest_Generator extends Producer_p_producer_GumboX_TestHarness_ScalaTest
  ...
  def getInitialiseProfiles: MSZ[Producer_p_producer_Profile]

  def getDefaultInitialiseProfile: Producer_p_producer_Profile = {
    return Producer_p_producer_Profile (
      name = "Default Initialise Profile",
      numTests = 100)
  }

  def getProfiles_P: MSZ[Producer_p_producer_Profile_P]

  def getDefaultProfile_P: Producer_p_producer_Profile_P = {
    return Producer_p_producer_Profile_P (
      name = "Default Port Profile", 
      numTests = 100, 
      numTestVectorGenRetries = 100)
  }

  def next(profile: Producer_p_producer_Profile_P): Option[Producer_p_producer_PreState_Container_P] = {
    try {
  ...
  def testInitialiseCB_Profile(profile: Producer_p_producer_Profile): Unit = {
  ...
  def testComputeCB_Profile_P(profile: Producer_p_producer_Profile_P): Unit = {
    for (i <- 0 until profile.numTests) {
        ...
        while (j < profile.numTestVectorGenRetries && retry) {
          next(profile) match {
            case Some(o) =>
              ...
              val json = prod_cons__JVM.JSON.fromProdConsFlowsProducer_p_producer_PreState_Container_P(o, T)
